...
class Reviewer:
    ...
    def _review_operations_with_llm(self, request: str, ops: list[dict]) -> dict:
        ...
        system_prompt = (
            ...
            "Given a user's multi-step instruction and the JSON operations generated by the assistant, "
            "determine if the operations completely satisfy EVERY part of the user's request.\n"
            "If the user asked for 5 things, ensure there are operations for all 5 things.\n"
            "Return JSON with:\n"
            "  - satisfied (bool): true ONLY if all requested changes are present.\n"
            "  - feedback (str): if false, clearly list WHICH instructions were ignored or missed by the assistant so it can fix them.\n"
            "Return ONLY valid JSON."
        )
