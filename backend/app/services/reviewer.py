...
class Reviewer:
    ...
    def review_operations(self, request: str, ops: list[dict]) -> dict:
        """Review generated operations.
        
        Checks if the generated operations completely satisfy ALL parts of a multi-step prompt.
        """
        if not ops:
            return {"satisfied": False, "feedback": "No operations were generated."}

        if settings.openai_api_key:
            try:
                return self._review_operations_with_llm(request, ops)
            except Exception:
                pass
    ...
    def _review_operations_with_llm(self, request: str, ops: list[dict]) -> dict:
        from openai import OpenAI
        client = OpenAI(
            api_key=settings.openai_api_key,
            base_url=settings.openai_base_url or None,
        ...
        system_prompt = (
            "You are a STRICT QA reviewer for a document editing assistant.\n"
            "Given a user's instruction (which may contain multiple sub-tasks) and the JSON "
            "operations generated by the assistant, determine if the operations completely "
            "satisfy EVERY SINGLE part of the user's request.\n\n"
            "RULES:\n"
            "- First, enumerate ALL distinct sub-tasks in the user's request (e.g. '1. Move section, "
            "2. Convert list, 3. Normalize headings').\n"
            "- For each sub-task, check whether at least one operation in the list directly addresses it.\n"
            "- A sub-task is ONLY satisfied if the operation type AND target are appropriate for it.\n"
            "- If ANY sub-task has zero matching operations, satisfied MUST be false.\n"
            "- Do NOT give the benefit of the doubt. Be strict.\n"
            "- An operation that is present but targets the wrong element counts as MISSING.\n\n"
            "Return JSON with:\n"
            "  - satisfied (bool): true ONLY if every sub-task has at least one correct operation.\n"
            "  - feedback (str): if false, clearly describe WHICH sub-tasks are missing or wrong.\n"
            "  - missed_tasks (list[str]): a short label for each unaddressed sub-task (empty list if satisfied).\n"
            "Return ONLY valid JSON."
        )
        ...
        if not isinstance(missed, list):
            missed = []
        return {
            "satisfied": bool(data.get("satisfied", False)),  # strict: default False
            "feedback": str(data.get("feedback", "")),
            "missed_tasks": [str(t) for t in missed],
        }
